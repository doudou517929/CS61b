...
public class RadixSort {
    ...
    private static void sortHelperLSD(String[] asciis, int index) {
        ...
        for (String s : asciis) {  // in this index, whose asciis are same, calculate num for every asciis
            counts[(int) s.charAt(index)]++;
        }
        int[] stars = new int[256];
        int startIndex = 1;
        stars[0] = 0;
        for (int i = 0; i < counts.length - 1; i++) { // add every num in counts, make every index stores the sum before it
            stars[startIndex] = stars[startIndex - 1] + counts[i];
            startIndex++;
        }
        String[] sorted = new String[asciis.length];
        for (int i = 0; i < asciis.length; i++) {
            int pos = stars[(int) asciis[i].charAt(index)];
            sorted[pos] = asciis[i];
            stars[(int) asciis[i].charAt(index)]++;
        }
